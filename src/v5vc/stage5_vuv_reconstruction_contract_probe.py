...
class ReconstructionContractVariant:
    label: str
    description: str
    window_kind: str
...
RECONSTRUCTION_CONTRACT_VARIANTS = [
    ReconstructionContractVariant(
        label="hann_window_sum_norm",
        description="Current baseline: Hann synthesis window with window-sum normalization.",
        window_kind="hann",
        normalization_kind="window_sum",
    ),
    ReconstructionContractVariant(
        label="hann_overlap_count_norm",
        description="Hann synthesis window with plain overlap-count normalization.",
        window_kind="hann",
        normalization_kind="overlap_count",
    ),
    ReconstructionContractVariant(
        label="hann_window_square_norm",
        description="Hann synthesis window with window-square normalization.",
        window_kind="hann",
        normalization_kind="window_square",
    ),
    ReconstructionContractVariant(
        label="sqrt_hann_window_square_norm",
        description="Square-root Hann synthesis window with window-square normalization.",
        window_kind="sqrt_hann",
        normalization_kind="window_square",
    ),
    ReconstructionContractVariant(
        label="rectangular_overlap_count_norm",
        description="Rectangular overlap averaging positive control.",
        window_kind="rectangular",
        normalization_kind="overlap_count",
    ),
]


def analyze_stage5_nores_vuv_reconstruction_contract_review(
    *,
    output_dir: Path,
    ...
    for review_record in review_records:
        ...
        if frame_count <= 0:
            raise ValueError(f"No waveform frames available for reconstruction-contract review record: {record_id}")
        ...
        if conditioning is None:
            raise ValueError(f"Unable to derive vuv conditioning for reconstruction-contract review record: {record_id}")
        ...
        for variant in RECONSTRUCTION_CONTRACT_VARIANTS:
            reconstructed_waveform = reconstruct_with_contract_variant(
            ...
            variant_rows.append(
                {
                    ...
                    "window_kind": variant.window_kind,
        ...
        diagnosis = diagnose_record_reconstruction_contract(
    ...
    summary = {
        ...
        "analysis_type": "stage5_vuv_reconstruction_contract_review_v1",
        "review_bundle_path": review_bundle_path.as_posix(),
        ...
        "aggregates": build_reconstruction_contract_aggregates(per_record_rows),
        "diagnosis": diagnose_reconstruction_contract_review(per_record_rows),
        "records": per_record_rows,
        "notes": [
            "This probe keeps waveform_frames fixed and only changes reconstruction window and normalization rules.",
            "hann_window_sum_norm reproduces the current no-gate decode contract.",
            "rectangular_overlap_count_norm is the positive control from the earlier geometry probe.",
        ],
    }
    json_path = output_dir / "stage5_vuv_reconstruction_contract_review.json"
    md_path = output_dir / "stage5_vuv_reconstruction_contract_review.md"
    json_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8", newline="\n")
    md_path.write_text(render_reconstruction_contract_review_markdown(summary), encoding="utf-8", newline="\n")


def build_reconstruction_window(*, frame_length: int, kind: str, dtype: torch.dtype, device: torch.device) -> torch.Tensor:
    if kind == "hann":
        return torch.hann_window(int(frame_length), periodic=False, dtype=dtype, device=device)
    if kind == "sqrt_hann":
        return torch.hann_window(int(frame_length), periodic=False, dtype=dtype, device=device).clamp_min(0.0).sqrt()
    if kind == "rectangular":
        return torch.ones((int(frame_length),), dtype=dtype, device=device)
    raise ValueError(f"Unsupported reconstruction window kind: {kind}")


def reconstruct_with_contract_variant(
    *,
    waveform_frames: torch.Tensor,
    frame_length: int,
    hop_length: int,
    variant: ReconstructionContractVariant,
) -> torch.Tensor:
    ...
    output = frames.new_zeros((total_length,))
    weights = frames.new_zeros((total_length,))
    window = build_reconstruction_window(
        frame_length=frame_length,
        kind=variant.window_kind,
        dtype=frames.dtype,
        device=frames.device,
    )
    if variant.normalization_kind == "window_sum":
        weight_kernel = window
    elif variant.normalization_kind == "window_square":
        weight_kernel = window.pow(2)
    elif variant.normalization_kind == "overlap_count":
        weight_kernel = torch.ones_like(window)
    else:
        raise ValueError(f"Unsupported reconstruction normalization kind: {variant.normalization_kind}")
    for frame_index in range(frame_count):
        start = int(frame_index * hop_length)
        end = start + int(frame_length)
        output[start:end] += frames[frame_index] * window
        weights[start:end] += weight_kernel
    return output / weights.clamp_min(1.0e-6)


def diagnose_record_reconstruction_contract(
    *,
    ...
    hann_sum = float(variant_map.get("hann_window_sum_norm", 0.0))
    hann_count = float(variant_map.get("hann_overlap_count_norm", 0.0))
    hann_square = float(variant_map.get("hann_window_square_norm", 0.0))
    sqrt_hann_square = float(variant_map.get("sqrt_hann_window_square_norm", 0.0))
    rectangular = float(variant_map.get("rectangular_overlap_count_norm", 0.0))
    if aligned_gap > 0.0 and waveform_gap <= 0.0:
        primary = "waveform_frames_vuv_not_ready_for_contract_probe"
    elif waveform_gap > 0.0 and rectangular > 0.0 and max(hann_count, hann_square, sqrt_hann_square) <= 0.0:
        primary = "hann_window_family_itself_is_main_sink"
    elif waveform_gap > 0.0 and max(hann_count, hann_square, sqrt_hann_square) > 0.0:
        primary = "hann_normalization_rule_partially_recovers_vuv_without_abandoning_hann"
    elif waveform_gap > 0.0 and rectangular > hann_sum:
        primary = "rectangular_contract_outperforms_all_current_hann_neighbors"
    else:
        primary = "needs_manual_review"
    secondary: list[str] = []
    if hann_count > hann_sum:
        secondary.append("hann_overlap_count_better_than_current_hann")
    if hann_square > hann_sum:
        secondary.append("hann_square_norm_better_than_current_hann")
    if sqrt_hann_square > hann_sum:
        secondary.append("sqrt_hann_square_norm_better_than_current_hann")
    if rectangular > hann_sum:
        secondary.append("rectangular_better_than_current_hann")
    return {
...
def build_reconstruction_contract_aggregates(records: list[dict[str, object]]) -> dict[str, object]:
    aligned = []
    waveform = []
    variant_values: dict[str, list[float]] = {variant.label: [] for variant in RECONSTRUCTION_CONTRACT_VARIANTS}
    for record in records:
        aligned.append(
            float(dict(record.get("vuv_summaries", {})).get("aligned", {}).get("unvoiced_minus_voiced_high_band_ratio", 0.0))
        )
        waveform.append(
            float(dict(record.get("vuv_summaries", {})).get("waveform_frames", {}).get("unvoiced_minus_voiced_high_band_ratio", 0.0))
        )
...
def diagnose_reconstruction_contract_review(records: list[dict[str, object]]) -> dict[str, object]:
    primary_counts: dict[str, int] = {}
    for record in records:
        primary = str(dict(record.get("diagnosis", {})).get("primary_localization", "missing"))
        primary_counts[primary] = primary_counts.get(primary, 0) + 1
    aggregates = build_reconstruction_contract_aggregates(records)
    waveform_mean = float(aggregates["waveform_frames_vuv_high_band_ratio_mean"])
    hann_sum = float(aggregates["hann_window_sum_norm_vuv_high_band_ratio_mean"])
    hann_count = float(aggregates["hann_overlap_count_norm_vuv_high_band_ratio_mean"])
    hann_square = float(aggregates["hann_window_square_norm_vuv_high_band_ratio_mean"])
    sqrt_hann_square = float(aggregates["sqrt_hann_window_square_norm_vuv_high_band_ratio_mean"])
    rectangular = float(aggregates["rectangular_overlap_count_norm_vuv_high_band_ratio_mean"])
    if waveform_mean <= 0.0:
        primary = "waveform_frames_vuv_not_ready_for_contract_probe"
    elif rectangular > 0.0 and max(hann_count, hann_square, sqrt_hann_square) <= 0.0:
        primary = "hann_window_family_itself_is_main_sink"
    elif max(hann_count, hann_square, sqrt_hann_square) > 0.0:
        primary = "hann_normalization_rule_partially_recovers_vuv_without_abandoning_hann"
    elif rectangular > hann_sum:
        primary = "rectangular_contract_outperforms_all_current_hann_neighbors"
    else:
        primary = "needs_manual_review"
    return {
...
def render_reconstruction_contract_review_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Stage5 Vuv Reconstruction-Contract Review Probe",
        "",
        "## Summary",
    ...
    for key in [
        ...
        "hann_window_sum_norm_vuv_high_band_ratio_mean",
        "hann_overlap_count_norm_vuv_high_band_ratio_mean",
        "hann_window_square_norm_vuv_high_band_ratio_mean",
        "sqrt_hann_window_square_norm_vuv_high_band_ratio_mean",
        "rectangular_overlap_count_norm_vuv_high_band_ratio_mean",
        "waveform_frames_vuv_nonpositive_count",
        "hann_window_sum_norm_vuv_nonpositive_count",
        "hann_overlap_count_norm_vuv_nonpositive_count",
        "hann_window_square_norm_vuv_nonpositive_count",
        "sqrt_hann_window_square_norm_vuv_nonpositive_count",
        "rectangular_overlap_count_norm_vuv_nonpositive_count",
    ]:
    ...
    lines.extend(
        [
            "",
            "## Notes",
            "- This probe holds waveform_frames fixed and changes only reconstruction window and normalization rules.",
            "- `hann_window_sum_norm` is the current no-gate decode contract.",
            "- `rectangular_overlap_count_norm` is the positive control from the earlier geometry probe.",
        ]
    )
    return "\n".join(lines) + "\n"
