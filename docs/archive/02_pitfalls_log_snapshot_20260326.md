...
- 处理要求:
  ...
  - 不允许继续做：
    ...
    - `hidden only`
      的平移变体
### 457. 不能把“每次上游 candidate 都先在 Stage3 变好”误解成“必须立刻再送回当前已判死的 Stage5 route”
- 现象:
  - 这轮新的
...
- 处理要求:
  ...
  - 更合理的顺序应改成：
    - 先在仍有效的层
      把证据做硬
      例如：
      `12-step -> 24-step full-validation`
    - 再决定：
      是否值得切换 handoff layer
  - 当前 case
    的正确动作是：
    - 把
      `acoustic_directional_transition_bridge_v1`
...
### 458. 不能把“停止当前旧 Stage5 route”误写成“以后不再做任何 Stage5 验证”
- 现象:
  - 在对
    `1.md`
    的一次性评估里，
...
### 459. 不能把“Stage5 仍失败”简化成“它还只是长期消费旧 heuristic event family”
- 现象:
  - 在外部总结口径里，
    很容易把当前 Stage5 的问题
    简化成：
