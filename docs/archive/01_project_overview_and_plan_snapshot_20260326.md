...
2. 若继续保留主线，
   ...
   - 或重新评估
     ...
     才真正能承接
     上游正向改动
## 2026-03-26 继续补充：Stage3 `teacher_e_evt` directional transition bridge 成为新的 generation-side reference
- 正式报告：
...
3. full-validation step12
   已经正向：
   - `loss_total:
      1.65222 -> 1.635463`
   - `loss_total_semantic_disabled_reference:
      1.54467 -> 1.529686`
4. 而且 full-validation step24
   仍保持正向，
   不是 12-step 偶然抖动：
   - `loss_total:
      0.975202 -> 0.947703`
   ...
   - `loss_teacher_event_prior:
      0.505388 -> 0.486579`
5. 这说明：
   - 当前上游 generation-side
     的最强 reference
     已从
     `acoustic_contextual`
     升级到
     `acoustic_directional_transition`
...
1. 现在可以更明确地说：
   - Stage3
     ...
     loss-side imitation
   - 也不是
     Stage5 consumer / decode 小修
   - 而是
     teacher-label generation-side
     对前 5 维 acoustic event bridge
     的结构升级
2. 也必须同时写清楚：
   - 这轮不应再把同一 candidate
     顺手送回
...
2. 若继续保留主线，
   ...
   - 或重新识别
     ...
     才真正能承接
     当前这些上游正向改动
## 2026-03-26 继续补充：`1.md` 一次性评估完成，原则可采纳但需修正表述边界
- 正式报告：
