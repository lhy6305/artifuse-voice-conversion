...
## Pitfall 111: On the upper-bound waveform-code route, lower brightness and better RMS do not by themselves mean the checkpoint kept the useful structure
- After `569`, the held-out `wavepca16` route produced two very different operating points:
  - plain `step40`: still too bright, but with `decoded_frame_rms_to_aligned_frame_rms_corr = 0.509279` and `decoded_frame_template_cosine_gap_vs_aligned = 0.429025`
  - `rms_guard=0.2 + bhb01 step40`: much darker and better-RMS (`decoded_spectral_centroid_hz = 7319.660156`, `decoded_spectral_high_band_energy_ratio = 0.489306`, `decoded_to_aligned_rms_ratio = 0.776896`), but `decoded_frame_rms_to_aligned_frame_rms_corr` collapsed to `0.084554`
- What goes wrong if this is ignored:
  - the regularized checkpoint can look like a clean win if the read tracks only harshness, centroid, high-band ratio, or RMS overshoot
  - that read misses the fact that the checkpoint may have suppressed the very frame-level geometry that made the upper-bound route valuable
- Operational rule:
  - on waveform-code upper-bound routes, always read brightness/RMS metrics together with:
    - `decoded_frame_rms_to_aligned_frame_rms_corr`
    - `decoded_frame_template_cosine_gap_vs_aligned`
    - control-usage ranking for `z_art / event_probs / conditioning`
  - do not promote an upper-bound checkpoint if it only becomes darker or quieter by collapsing alignment-facing structure
